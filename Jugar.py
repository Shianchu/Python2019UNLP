...
def dimensiones(listaPal):
	"""Modulo el cual genera las dimensiones para la sopa de letras"""
	max=0
	for i in listaPal:
		if len(i) >= max:
			max= len(i)
...
def play(listita,cant,vert,caps,ayuda,color):
	"""Modulo el cual muestra los botones que se utilizaran en el juego al igual que el grafico de la sopa de letras"""
	lista=[]
	defi=[]
	tipo=[]
	cant2=[0,0,0]
